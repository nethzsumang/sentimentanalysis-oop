...
class blWordCloud:
    ...
    def generate_word_cloud():
        ...
        tobacco_word_tags_path = path_join('resources', 'storage', 'WordTagsTobacco.xls')

        vape_words = blWordCloud.read_file(vape_word_tags_path)
        tobacco_words = blWordCloud.read_file(tobacco_word_tags_path)

        wordcloud_vape = wordcloud.WordCloud(width=500,
                                             height=500,
                                             background_color='white',
                                             stopwords=set(wordcloud.STOPWORDS),
                                             min_font_size=10).generate(vape_words)

        wordcloud_tobacco = wordcloud.WordCloud(width=500,
                                             height=500,
                                             background_color='white',
                                             stopwords=set(wordcloud.STOPWORDS),
                                             min_font_size=10).generate(tobacco_words)
